  ...
  window.__submitSubjectRequest = async function () {
    ...
    try {
      ...
      if (isAdminOrEditor()) {
        ...
        // Mã môn đã có → chỉ thêm câu hỏi vào môn hiện tại
...
(function () {
  ...
  function renderStudyWithReport() {
    ...
    $('studyList').innerHTML = arr.slice(0, max).map(c => {
      const auto = !!c.__autoOpenAnswer; return `<div class="sitem compactStudyCard ${auto ? 'autoOpenAnswer open' : ''}" data-num="${esc(c.num)}" tabindex="0">
      <div class="compactCardLine">
        <div class="compactCardMeta"><span class="snum compactSubject">CÂU ${esc(c.num)}</span></div>
        <div class="sq compactQuestionText">${markText(c.question, q, 'phraseMark')}</div>
...
(function () {
  ...
  function setup() {
    ...
    const subjects = typeof window.getSubjectsCache === 'function' ? window.getSubjectsCache() : [];
    const activeSubject = localStorage.getItem('learninghub_subject_code_merged_v1') || '';
    ...
    const buildChip = (s) => {
      ...
      return `
        <label class="examSubjectChip ${checked ? 'checked' : ''}" data-exam-subj="${E(s.code)}">
          ...
          <span class="examSubjectChipCode">${E(s.code)}</span>
          <span class="examSubjectChipName">${E(s.name || '')}</span>
          ${countText ? `<span class="examSubjectChipCount">${countText}</span>` : ''}
        </label>`;
    };

    // Active subject card (full info like subject gate card)
    const activeSub = subjects.find(s => s.code === activeSubject) || (activeSubject ? { code: activeSubject, name: activeSubject } : null);
    ...
    const activeCard = activeSub ? `
      <div class="examActiveSubjectCard">
        <div class="examActiveSubjectTop">
          <span class="examActiveSubjectCode">${E(activeSub.code)}</span>
          <span class="examActiveSubjectName">${E(activeSub.name || activeSub.code)}</span>
        </div>
        <div class="examActiveSubjectDesc">${activeDesc}</div>
  ...
  function draw() {
    ...
    if (!qSet || !qSet.length) {
      ...
      body.innerHTML = '<div class="examOnlyEmpty">Chọn số câu rồi bấm <b>Bắt đầu kiểm tra</b>.</div>';
      return;
    }
