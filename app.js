...
window.HODSupabase = (() => {
  ...
  function openAdmin(){ if (!isAdmin()) { $id('adminModal')?.classList.add('hidden'); alert('Tài khoản Google này chưa có quyền admin.'); return; } $id('adminModal')?.classList.remove('hidden'); loadPendingRequests(); }
  ...
  async function loadProfile(){
    ...
    if (error) {
      console.warn(error);
      currentProfile = { id: currentUser.id, email: currentUser.email, role: 'user' };
...
(function(){
  ...
  function login(){
    ...
    if(!api){ alert('Supabase chưa sẵn sàng, hãy tải lại trang.'); return; }
    if(api.signInGoogle){ api.signInGoogle(); return; }
  ...
  function openDash(){ if(isAdmin()) window.HODSupabase?.openAdmin?.(); else alert('Tài khoản này không có quyền admin.') }
  ...
  function updateAll(){ ensureAvatar(); const u=user(); const admin=isAdmin(); document.body?.classList.toggle('hod-is-admin-final',admin); if(u) hideLogin(); else showLogin(); const top=$('hodTopAvatar'); if(top){top.innerHTML=avatarHTML(); top.style.display=u?'grid':'none'} const headerAdmin=$('adminOpenBtn'); if(headerAdmin){headerAdmin.classList.toggle('hidden',!admin); headerAdmin.style.display=admin?'':'none'} if(!admin)$('adminModal')?.classList.add('hidden'); updateMenu() }
